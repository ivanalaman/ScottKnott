                    ...
                    ...)
{
  ...
  if(dim(m.res)[1] != 1) {
    ...
    id.groups <- c(apply(m.res, 2, paste, collapse = ''))
  } else {
    id.groups <- m.res
  }
  ...
  if(is.null(ylim))
    if(replicates)
      ylim <- c(.90 * min(minmax[, 1]), max(minmax[, 2]))
    else
  ...
  switch(match.arg(dispersion),
         mm = {
           segments(groups, minmax[,1], groups, minmax[,2],
                    lty = d.lty, col = d.col, ...)
         },
         sd = {
           segments(groups, sdd[,1], groups, sdd[,2],
                    lty = d.lty, col = d.col, ...)
         },
         ci = {
           segments(groups, ic[,1], groups, ic[,2],
                    lty = d.lty, col = d.col, ...)
         },
         cip = {
  ...
  if(axisy) {
    axis(2,
         at = round(seq(ylim[1], ylim[2], length.out = 5), 1))
  }

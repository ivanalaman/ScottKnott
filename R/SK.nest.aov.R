                        ...
                        sig.level=.05, ...)
{
  ...
  if(fl3 == 0)                        # bifactorial                  
  {
    ...
    m.inf  <- matrix(nrow=l,
                     ncol=3)
    for(i in 1:l){ 
      v <- x$model[, 1][(x$model[,which2] == levels(x$model[,which2])[i])
                        &(x$model[,which1] == levels(x$model[,which1])[fl2])] 
      m.inf[i, 1] <- mean(v)
      m.inf[i, 2] <- min(v)
      m.inf[i, 3] <- max(v)
    }
    f2 <- paste(strtrim(which1,
                        id.trim),
                fl2,
                sep='_')
    f1 <- paste(strtrim(which2,
                        id.trim),
                1:l,
                sep='_')
    fl <- tolower(paste(f2,
  ...
  {
    m      <- c(tab[fl2,fl3,1], tab[fl2,fl3,2])  #trifactorial
    which1 <- names(dimnames(tab))[3]
    which2 <- names(dimnames(tab))[2]  
    which3 <- names(dimnames(tab))[1]
    l      <- nlevels(x$model[,which1])
    m.inf  <- matrix(nrow=l,
                     ncol=3)
    for(i in 1:l){ 
    ...
    f1 <- paste(strtrim(which1,
                        id.trim),
                1:l,
                sep='_')
    f2 <- paste(strtrim(which2,
                        id.trim),
                fl2,
                sep='_')
    f3 <- paste(strtrim(which3,
                        id.trim),
                fl3,
                sep='_')
    fl <- tolower(paste(f3,
